...
- **Phase 4**, Cross-user 2nd-degree warm-intro graph, THE DIFFERENTIATOR (`docs/specs/V2_P4_WARM_INTROS.md`): consent-gated (`users.share_network_for_intros`, default off; `users.linkedin_urn`). `GET /api/warm-intros` (service-role) finds bridge peers B (opted-in + connected to A) → B's contacts matching A's goal, minus already-known → "via {peer}" cards. Sidebar "Warm Intros" → `/v2/warm-intros` lane + Settings opt-in toggle + one-tap "Ask {peer} for an intro" draft (reuses generate). **Verified end-to-end on seeded 2-user data** (matcher, lane, draft). Real-user gap: capture each user's OWN urn at sync (`docs/specs/V2_P4_URN_CAPTURE_FOLLOWUP.md`) + needs ≥2 real opted-in connected users (GTM). Scale TODO documented in the route.
- **Discovery UX**: refine chat detects "find people at [company]" → offers a "Run live search at {company}" button (opens company-discovery prefilled); queue capped at 16 (two rows of 8) with cohesive cream tokens + initials avatars (no rainbow / blank circles); card no longer overlaps the queue banner; full-profile experience/education collapses with show-more.
- **Company intel**, meeting prep wired to **Brave Search** (`BRAVE_API_KEY`, free tier) → real recent dated news fed to MiniMax to synthesize (Perplexity fallback; graceful-empty if no key). Verified live.
- **Routing**, `/v2` is the default app (root + post-login + authed-user middleware `APP_HOME` → `/v2`; extension popup links → `/v2`). thewarmly.com active (domain verified, email login works, Supabase redirect URLs + extension origin added).
- **Polish**, draft-refusal fixed (`profile_md` now wired into the outreach prompt); em-dashes stripped from scoring rationale; onboarding step bar hidden on transitional screens.
...
The full VISION journey is now built + verified end to end: **CV → profile → goal-filtered scored discovery (+ live company search) → save → draft → meeting prep + real company intel → synthesized notes → CRM reconnect → cross-user warm intros.** Remaining gaps are user/extension-dependent (live scrape + sync need an extension reload; real warm intros need ≥2 real opted-in users; Google sign-in needs a Google OAuth client; INSEAD on-campus domain needs an IT allowlist).
