...
export interface GenerationContext {
  ...
  /**
   * User's auto-built free-form identity narrative. Generated by
   * `lib/ai/profile.ts` from onboarding answers + optional CV upload +
   * ongoing conversation enrichment. Carries Liyang-specific (or any
   * user-specific) hooks, transition story, and voice that don't fit
   * into structured columns. Null/undefined until onboarding completes.
   * Optional in the type so older call sites that don't yet pass it
   * remain backwards compatible.
   */
  user_profile_md?: string | null;
  /**
   * Approved learnings from prior sent messages — Phase C of the
   * self-improvement loop. Each is a generalizable pattern the agent
   * has learned from how the user actually edits and sends drafts.
   * Injected into outreach prompts to make every new draft progressively
   * more aligned with the user's voice and strategy.
   */
